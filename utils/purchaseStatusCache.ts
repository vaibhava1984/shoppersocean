...
function cacheKey(userId: string, productId: string) { return `${userId}:${productId}` }
async function flush(userId: string) {
    ...
    try {
        ...
        productIds.forEach(productId => {
            ...
            statusCache.set(cacheKey(userId, productId), { hasPurchased, expiresAt: Date.now() + (hasPurchased ? POSITIVE_CACHE_MS : NEGATIVE_CACHE_MS) })
            pending.get(cacheKey(userId, productId))?.forEach(({ resolve }) => resolve(hasPurchased)); pending.delete(cacheKey(userId, productId))
        })
    } catch (error) {
        productIds.forEach(productId => { const key = cacheKey(userId, productId); pending.get(key)?.forEach(({ reject }) => reject(error)); pending.delete(key) })
    }
}
export function getPurchaseStatus(userId: string, productId: string): Promise<boolean> {
    const key = cacheKey(userId, productId); const cached = statusCache.get(key)
    if (cached && cached.expiresAt > Date.now()) return Promise.resolve(cached.hasPurchased)
    if (cached) statusCache.delete(key)
    const promise = new Promise<boolean>((resolve, reject) => { const requests = pending.get(key) ?? []; requests.push({ resolve, reject }); pending.set(key, requests) })
    const productIds = queuedByUser.get(userId) ?? new Set<string>(); productIds.add(productId); queuedByUser.set(userId, productIds)
    if (!scheduledUsers.has(userId)) { scheduledUsers.add(userId); setTimeout(() => { void flush(userId) }, 0) }
    return promise
}
