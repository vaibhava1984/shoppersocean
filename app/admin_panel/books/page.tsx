...
export default function Books() {
    ...
    async function fetchBooks() {
        try {
            ...
            const { data, error } = await supabase.from('books').select(`id,title,description,published_date,isbn,price,ratings,cover_images,binding,language,publisher,pages,author_id,author_name,updated_at,authors (name)`).eq('is_deleted', false);
    ...
    async function handleDeleteBook(book: BookType) {
        try {
            const supabase = createClient();
            const { error } = await supabase.from('books').update({ is_deleted: true }).eq('id', book.id);
            if (error) throw error;
            fetchBooks(); setShowDeleteAlert(false); setBookToDelete(null);
            ...
            <div className="flex h-screen bg-gray-100">
                ...
                <main className="flex-1 overflow-y-auto p-8">
                    <div className="space-y-6">
                        <Card><CardHeader><CardTitle>Books Management</CardTitle></CardHeader><CardContent>
                            ...
                            <Table><TableHeader><TableRow>{Array.from(selectedColumns).map((column, i) => <TableHead key={`${column}_${i}`}>{availableColumns.find(col => col.key === column)?.label}</TableHead>)}<TableHead>Actions</TableHead></TableRow></TableHeader><TableBody>{displayedBooks.map(book => <TableRow key={book.id}>{Array.from(selectedColumns).map((column, i) => <TableCell key={`${column}_${i}`}>{column === "author_name" ? book?.authors?.name ?? `${book[column]} 000` : book[column]}</TableCell>)}<TableCell><Button onClick={() => { setSelectedBook(book); setShowEditForm(true); }} className="mr-2">Edit</Button><Button onClick={() => { setBookToDelete(book); setShowDeleteAlert(true); }} variant="destructive">Delete</Button></TableCell></TableRow>)}</TableBody></Table>
                        </CardContent></Card>

                        <Dialog open={showAddForm} onOpenChange={setShowAddForm}><DialogContent className="max-h-[80vh] overflow-y-auto"><DialogHeader><DialogTitle>Add New Book</DialogTitle></DialogHeader>{showAddForm && <AddBookPopup onSuccess={(closePopup) => { fetchBooks(); if (closePopup) setShowAddForm(false); }} />}</DialogContent></Dialog>
                        <Dialog open={showEditForm} onOpenChange={setShowEditForm}><DialogContent className="max-h-[80vh] overflow-y-auto"><DialogHeader><DialogTitle>Edit Book</DialogTitle></DialogHeader>{showEditForm && selectedBook && <AddBookPopup book={selectedBook} onSuccess={() => { fetchBooks(); setShowEditForm(false); }} />}</DialogContent></Dialog>
