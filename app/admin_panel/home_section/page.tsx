...
function BookSection({ title, entries, maxBooks, sectionType, onAdded, onRemoved }: BookSectionProps) {
    ...
    return <Card className="mb-8"><CardHeader><CardTitle>{title}</CardTitle><CardDescription>Select up to {maxBooks} books for this section</CardDescription></CardHeader><CardContent>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}><DialogContent><DialogHeader><DialogTitle>Search Books</DialogTitle></DialogHeader><div className="space-y-4">
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto">{isSearching ? <div className="py-3 text-sm text-muted-foreground">Searching...</div> : searchQuery.trim().length >= 2 && searchResults.length === 0 ? <div className="py-3 text-sm text-muted-foreground">No books found.</div> : searchResults.map((book) => <button key={book.id} type="button" disabled={isSaving} className="flex min-h-12 w-full cursor-pointer items-center justify-start rounded-md border border-input bg-background px-4 py-3 text-left text-sm font-medium shadow-sm transition-colors hover:bg-accent hover:text-accent-foreground disabled:pointer-events-none disabled:opacity-50" onPointerDown={(event) => { event.preventDefault(); event.stopPropagation() }} onClick={(event) => { event.preventDefault(); event.stopPropagation(); void handleBookSelect(book) }}><div className="text-left"><div className="font-medium">{book.title}</div><div className="text-sm text-muted-foreground">{book.author || 'Unknown author'}</div></div></button>)}</div>
