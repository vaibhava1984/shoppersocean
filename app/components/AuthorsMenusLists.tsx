    ...
    return (
        <div className="flex flex-col space-y-2">
            {initialAuthors.map((author) => {
                ...
                return (
                    <Button key={author.author_id} asChild variant={currentAuthor === author.author_id ? "default" : "ghost"} className="h-11 justify-start w-full rounded-lg px-4 font-sans text-base font-bold tracking-normal text-green-700 transition-colors hover:bg-green-50 hover:text-green-800">
                        <Link href={"/bookShelf?" + params.toString()}>{author.name}</Link>
                    </Button>
                )
            })}
        </div>
