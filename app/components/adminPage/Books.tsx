    ...
    async function handleDeleteBook(book: BookType) {
        try {
            ...
            const { data: relatedData, error: checkError } = await supabase
                .from('related_tables') // Replace with your actual related table name
                .select('id')
                .eq('book_id', book.id)
                .limit(1);
            ...
            if (relatedData && relatedData.length > 0) {
                alert(`Cannot delete book "${book.title}" because it has related records. Please delete those records first.`);
                return;
            }
