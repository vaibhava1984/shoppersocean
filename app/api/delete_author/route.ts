...
export async function POST(request: Request) {
  try {
    ...
    const books = await firestore.collection("books").where("author_id", "==", author_id).where("is_deleted", "==", false).get();
    if (!books.empty) return NextResponse.json({ error: "Please delete the books associated with this author." }, { status: 500 });

    await firestore.collection("authors").doc(String(author_id)).set({ is_deleted: true, updated_at: new Date().toISOString() }, { merge: true });
