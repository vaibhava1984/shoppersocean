...
export async function GET() {
  try {
    ...
    const wanted=new Set(ids),snap=await firestore.collection('books').get()
    const byId=new Map(snap.docs.filter(d=>wanted.has(d.id)&&d.data().is_deleted!==true).map(d=>{const b:any=d.data();return [d.id,{id:d.id,title:b.title??'',description:b.description??'',coverImage:Array.isArray(b.cover_images)?b.cover_images[0]:undefined,images:Array.isArray(b.cover_images)?b.cover_images:undefined,author:b.author_name??b.author??'',price:Number(b.price??0)}]}))
