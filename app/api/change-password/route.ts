...
export async function POST(request: Request) {
  try {
    ...
    if (passwordError) {
      ...
      return NextResponse.json({ error: passwordError.message || "Unable to change your password." }, { status: 422 })
    }
    ...
    const safeName = String(user.user_metadata?.full_name || "Customer")
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;")
    ...
    if (emailError) {
      ...
      return NextResponse.json({
        success: true,
        emailSent: false,
        message: "Your password has been changed successfully, but the confirmation email could not be sent.",
      })
    }
    ...
    return NextResponse.json({
      success: true,
      emailSent: true,
      message: "Your password has been changed successfully. A confirmation email has been sent to your email address.",
    })
  } catch (error) {
