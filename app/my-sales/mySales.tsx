...
const AuthorOrdersDashboard = ({ authorId }: { authorId: string }) => {
    ...
    const getFilteredDate = (filterType) => {
        ...
        switch (filterType) {
            ...
            case 'untilnow': return new Date(0).toISOString();
            default: return null;
        }
    };
    ...
    const getSalesData = async (authorId, timeFilter = null) => {
        ...
        try {
            ...
            if (timeFilterDate) ordersQuery = ordersQuery.gte('created_at', timeFilterDate);
            ...
            {loading ? (
                <div className="p-6 bg-gray-50">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        {[1, 2, 3].map((i) => <div key={i} className="bg-white rounded-lg shadow p-6"><Skeleton className="h-4 w-24 mb-2" /><Skeleton className="h-8 w-16" /></div>)}
                    </div>
                    <div className="bg-white rounded-lg shadow overflow-hidden">
                        <div className="px-6 py-4 border-b border-gray-200"><Skeleton className="h-6 w-32" /></div>
                        <div className="overflow-x-auto"><div className="p-6 space-y-4">{[1, 2, 3].map((i) => <div key={i} className="flex gap-4"><Skeleton className="h-4 w-1/3" /><Skeleton className="h-4 w-24" /><Skeleton className="h-4 w-24" /><Skeleton className="h-4 w-32" /></div>)}</div></div>
                    </div>
                </div>
            ) : (
                <div className="p-6 bg-gray-50">
                    ...
                    <div className="bg-white rounded-lg shadow overflow-hidden">
                        <div className="px-6 py-4 border-b border-gray-200"><h2 className="text-lg font-medium text-gray-900">Sales Details</h2></div>
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {orderedBooksData.salesDetails?.map((sale) => <tr key={sale.bookId}>
