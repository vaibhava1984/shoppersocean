...
type UrlInfo = { downloadUrl: string; fileName: string; fileType: 'jpeg' | 'pdf'; };
...
function getExchangeRatesOnce(): Promise<ExchangeRates> {
    ...
    if (!exchangeRatesPromise) {
        exchangeRatesPromise = fetchExchangeRates().then(rates => { exchangeRatesCache.set(rates); return rates; }).catch(error => { exchangeRatesPromise = null; throw error; });
    }
...
export default function PaymentButton({ amount, notes, userId, productId }: PaymentButtonProps) {
    ...
    useEffect(() => {
        ...
        async function setupLocalCurrency(passedCurrency?: string) {
            try {
                ...
                } else { setLocalCurrency('INR'); setLocalAmount(amount); }
                setIsInitialFetching(false);
            } catch (error) {
                if (!active) return;
                console.error('Error setting up local currency:', error);
                setLocalCurrency('INR'); setLocalAmount(amount); setIsInitialFetching(false);
            }
        }
        if (userId) {
    ...
    useEffect(() => { if (productId && userId) checkPurchaseViaAPI(productId); }, [productId, userId]);
    useEffect(() => { if (productId && userId && hasPurchased) handleDownload(productId); }, [productId, userId, hasPurchased]);

    const checkPurchaseViaAPI = async (id: string) => {
        try { setIsInitialFetching(true); const response = await fetch('/api/check-purchase', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ productId: id }) }); const data = await response.json(); setHasPurchased(data.hasPurchased); }
        catch (error) { console.error('Error:', error); } finally { setIsInitialFetching(false); }
    };
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            const options = { key: process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID, amount: Math.round(localAmount * 100), currency: localCurrency, name: 'Shoppers Ocean', description: `Payment of ${localAmount} ${localCurrency}`, order_id: orderId,
                handler: async (paymentResponse: any) => { try { const verificationResponse = await fetch('/api/verify-payment', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ razorpay_order_id: paymentResponse.razorpay_order_id, razorpay_payment_id: paymentResponse.razorpay_payment_id, razorpay_signature: paymentResponse.razorpay_signature, original_currency: localCurrency, original_amount: localAmount, user_id: userId ?? '', product_id: productId, quantity: 1 }) }); const data = await verificationResponse.json(); if (data.error) alert(`Payment failed: ${data.errorDetails || data.error}`); else if (data.status === 'completed') { alert('Payment successful!'); setHasPurchased(true); handleDownload(productId); } else if (data.status === 'authorized') alert('Payment authorized, awaiting capture'); else if (data.status === 'pending') alert('Payment is pending'); else alert(`Payment status: ${data.status}`); } catch (error) { console.error('Error:', error); alert('Payment verification failed'); } },
                notes: { skip_contact_form: 1 }, theme: { color: '#F37254' } };
            const paymentObject = new (window as any).Razorpay(options); paymentObject.open();
        } catch (error) { console.error('Error:', error); toast({ variant: "destructive", title: "Payment Error", description: "Unable to process payment. Please try again." }); }
        finally { setIsLoading(false); }
    };

    async function handleDownload(bookId: string) { try { setIsFetchingDownloadUrls(true); const response = await fetch('/api/get-book-download', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ bookId }) }); const allData = await response.json(); if (!response.ok) return; setDownloadUrls(allData.urls); } catch (error: any) { console.error('Error downloading file:', error); toast({ variant: "destructive", title: "Error", description: error?.message ?? "Failed to fetch download links" }); } finally { setIsFetchingDownloadUrls(false); } }
    async function downloadFile(downloadData: { downloadUrl: string, fileName: string, fileType: string }) { try { const { downloadUrl: sourceUrl, fileName } = downloadData; if (!sourceUrl || !fileName) return; const response = await fetch(sourceUrl); const blob = await response.blob(); const downloadUrl = window.URL.createObjectURL(blob); const link = document.createElement('a'); link.href = downloadUrl; link.download = fileName; document.body.appendChild(link); link.click(); document.body.removeChild(link); window.URL.revokeObjectURL(downloadUrl); } catch (error) { console.error('Download failed:', error); } }
    const userLocale = typeof navigator !== 'undefined' && navigator.language ? navigator.language : 'en-US';
    ...
    if (hasPurchased) return (<><button onClick={() => downloadUrls?.length ? downloadFile(downloadUrls[0]) : alert("File not found!")} className="px-4 py-2 flex bg-blue-500 text-white rounded h-[40px] hover:scale-105 hover:shadow-lg active:scale-95 transition-all duration-200 disabled:bg-gray-400">{isFetchingDownloadUrls && <Loader2Icon width={20} className='animate-spin mr-2' />}<span>{isFetchingDownloadUrls ? 'Processing' : 'Download'}</span></button><AlertDialog open={isDownloadDialogOpen} onOpenChange={setIsDownloadDialogOpen}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Download File</AlertDialogTitle></AlertDialogHeader><AlertDialogDescription><div className="grid grid-cols-2 gap-4">{downloadUrls.map((downloadUrl, i) => <div key={`${i}_download_file`}><a href={downloadUrl?.downloadUrl} download={downloadUrl?.fileName} target="_blank" className="flex items-center space-x-2 rounded-sm px-4 py-3 hover:underline text-white bg-blue-400"><FileIcon width={24} /><span>{downloadUrl?.fileType?.toUpperCase()} file</span></a></div>)}</div></AlertDialogDescription><AlertDialogFooter><AlertDialogCancel>Close</AlertDialogCancel></AlertDialogFooter></AlertDialogContent></AlertDialog></>);
