...
export default function PaymentButton({ amount, notes, userId, productId, productTitle }: PaymentButtonProps) {
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            const response = await fetch('/api/create-order', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ productId, amount: localAmount, currency: localCurrency, notes: { ...notes, product_title: productTitle } }) });
            const { orderId, keyId } = await response.json();
            const options = { key: keyId, amount: Math.round(localAmount * 100), currency: localCurrency, name: 'Shoppers Ocean', description: `Payment of ${localAmount} ${localCurrency}`, order_id: orderId,
