...
export default function BookFlipbook({ bookId, title }: Props) {
  ...
  useEffect(() => { loadReader(); }, [loadReader]);
  ...
  const renderPage = useCallback(async () => {
    ...
    } catch (err: any) { if (err?.name !== 'RenderingCancelledException') setError('Unable to render this page'); }
  ...
  const changePage = (nextPage: number) => {
    ...
    setTurning(true);
    window.setTimeout(() => { setPage(nextPage); window.setTimeout(() => setTurning(false), 280); }, 60);
  };
  ...
  if (error && !readerUrl) return <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-red-700"><p className="font-semibold">Could not open this book</p><p className="mt-1 text-sm">{error}</p><Button className="mt-4" onClick={loadReader}>Try again</Button></div>;
  ...
  return <div ref={viewerRef} className={`overflow-hidden rounded-xl border bg-slate-900 text-white shadow-xl ${fullscreen ? 'flex min-h-screen flex-col' : ''}`}>
    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/10 bg-slate-950 px-3 py-2">
      ...
      <div className="flex items-center gap-1">
        ...
        <Button variant="ghost" className="gap-2 text-white hover:bg-white/10" onClick={downloadPdf} disabled={downloading}><Download className="h-4 w-4" /> <span className="hidden sm:inline">{downloading ? 'Preparing…' : 'Download as PDF book'}</span></Button>
