...
export default function BookFlipbook({ bookId, title }: Props) {
  ...
  const changePage = (next: number) => { if (next < 1 || next > pageCount || turning) return; setTurning(true); window.setTimeout(() => { setPage(next); window.setTimeout(() => setTurning(false), 300); }, 80); };
  ...
  const handleTouchEnd = (event: React.TouchEvent) => { const start = touchStartXRef.current; const end = event.changedTouches[0]?.clientX ?? null; touchStartXRef.current = null; if (start === null || end === null || Math.abs(end - start) < 45) return; changePage(end < start ? page + 1 : page - 1); };
  ...
  return <div ref={viewerRef} className={`overflow-hidden rounded-xl border bg-slate-900 text-white shadow-xl ${fullscreen ? 'flex min-h-screen flex-col' : ''}`}><div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/10 bg-slate-950 px-3 py-2"><div className="min-w-0 truncate font-medium">{title}</div><div className="flex items-center gap-1"><Button variant="ghost" size="icon" className="text-white hover:bg-white/10" onClick={() => setZoom(Math.max(0.75, Number((zoom - 0.15).toFixed(2))))} aria-label="Zoom out"><Minus /></Button><span className="w-12 text-center text-xs">{Math.round(zoom * 100)}%</span><Button variant="ghost" size="icon" className="text-white hover:bg-white/10" onClick={() => setZoom(Math.min(2.5, Number((zoom + 0.15).toFixed(2))))} aria-label="Zoom in"><Plus /></Button><Button variant="ghost" size="icon" className="text-white hover:bg-white/10" onClick={toggleFullscreen} aria-label={fullscreen ? 'Exit full screen' : 'Full screen'}>{fullscreen ? <X /> : <Expand />}</Button></div></div>{error && <div className="bg-amber-50 px-4 py-2 text-sm text-amber-900">{error}</div>}<div className={`flex flex-1 items-center justify-center overflow-auto p-3 sm:p-6 ${fullscreen ? 'min-h-0' : 'min-h-[520px]'}`} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}><div className="relative rounded bg-white shadow-2xl transition-all duration-300 ease-in-out" style={flipStyle}><canvas ref={canvasRef} className="block max-w-full" />{rendering && <div className="absolute inset-0 flex items-center justify-center bg-white/70 text-slate-700"><Loader2 className="animate-spin" /></div>}</div></div><div className="flex items-center justify-center gap-4 border-t border-white/10 bg-slate-950 px-3 py-3"><Button variant="ghost" className="text-white hover:bg-white/10" disabled={page <= 1} onClick={() => changePage(page - 1)} aria-label="Previous page"><ChevronLeft /> Previous</Button><span className="min-w-24 text-center text-sm">Page {page} / {pageCount || '—'}</span><Button variant="ghost" className="text-white hover:bg-white/10" disabled={page >= pageCount} onClick={() => changePage(page + 1)} aria-label="Next page">Next <ChevronRight /></Button></div></div>;
}
